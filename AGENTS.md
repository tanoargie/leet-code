...
cmake --build build -j
cmake --build build --target <n>   # build one problem
./build/<n>                # run that problem's tests
...
- Any `problems/<n>/test.cpp` is registered automatically as target `<n>`; re-run `cmake -S . -B build` after adding a test file.
